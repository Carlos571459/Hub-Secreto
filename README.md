...
screenGui.IgnoreGuiInset = true -- para mobile usar toda a tela
...
-- Ajusta posição e tamanho do botão para mobile ou desktop
if isMobile() then
    button.Size = UDim2.new(0, 50, 0, 50)
    button.Position = UDim2.new(0.5, 0, 1, -100) -- centro inferior móvel
else
    button.Size = UDim2.new(0, 60, 0, 60)
    button.Position = UDim2.new(0, 20, 0.5, -30) -- lado esquerdo centro desktop
end
...
-- Botão invisível para clique/hover
...
-- Animações e tamanhos ajustados para mobile/desktop
local originalButtonSize = button.Size
local hoverButtonSize = UDim2.new(0, button.Size.X.Offset + 5, 0, button.Size.Y.Offset + 5)
local clickButtonSize = UDim2.new(0, button.Size.X.Offset - 5, 0, button.Size.Y.Offset - 5)
...
-- Função para criar button com animações, adaptando tamanho para mobile
...
local function makeDraggable(frame, dragHandle)
    ...
    if isMobile() then
        -- Desabilitar drag em mobile para evitar conflito
        return
    end
...
clickDetector.MouseButton1Click:Connect(function()
    ...
    local function createContainers(parent)
        ...
        if isMobile() then
            tabContainer.Size = UDim2.new(1, -24, 0, 50)
        else
    ...
    local function addExampleContent()
        ...
        for i, contentData in pairs(contents) do
            ...
            if i == 1 then -- Scripts
                local executeBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "▶️ Executar Script", Color3.fromRGB(40, 167, 69))
                local loadBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "📁 Carregar Script", Color3.fromRGB(23, 162, 184))
            elseif i == 2 then -- Hubs
                local infiniteYieldBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "🚀 Infinite Yield", Color3.fromRGB(108, 117, 125))
                local darkHubBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "🌙 Dark Hub", Color3.fromRGB(52, 58, 64))
            elseif i == 3 then -- Stats
                local speedBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "⚡ Aumentar Speed", Color3.fromRGB(255, 193, 7), nil, Color3.fromRGB(0, 0, 0))
                local jumpBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "🦘 Aumentar Jump", Color3.fromRGB(255, 87, 34))
            elseif i == 4 then -- Configurações
                local themeBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
                    "🎨 Mudar Tema", Color3.fromRGB(156, 39, 176))
                local resetBtn = GuiManager:CreateButton(tabContents[i], UDim2.new(0, 180, 0, 35), nil,
